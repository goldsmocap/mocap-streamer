...
export function connectSink(upstream: string, sink: Sink): Promise<Connection[]> {
  const conn = connections.find(({ from, to }) => from === upstream && to === sink.name);
  if (conn) {
    return Promise.reject(`Connection already exists between ${upstream} and ${sink.name}`);
  }
